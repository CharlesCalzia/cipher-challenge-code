...
def decrypt_nihilist(ciphertext, key):
    alphabet=u'A B C D E F G H J Z K L M N O P Q R S T U V W X Y'.split(' ')
    try: 
...
def decrypt_nihilist2(ciphertext, key):
    alphabet=u'A B C D E F G H I Z K L M N O P Q R S T U V W X Y'.split(' ')
    poly = decrypt_polybius(alphabet, ciphertext)

def bruteforce_nihilist(ciphertext, lengths=list(range(2,9))): #needs work to convert it to copy subsitution bruteforce algo
    ...
    for length in lengths:
        ...
        for i in range(3):
            ...
            count = 0
            while count <1000:
                a, b = random.randint(0,length-1), random.randint(0,24) #select random letters to switch in the alphabet
                c=p2[:]
                c[a]=alphabet2[b]
            ...
            if pScore>mScore:
                mScore = pScore
                m = p2[:]
                print(f'Length {length} best key: {m} - {decrypt_nihilist(ciphertext,m)[:20]}')

def decrypt_playfair(alphabet, ciphertext):
    return Playfair(key=alphabet).decipher(ciphertext)

def bruteforce_playfair(ciphertext): #not really working
    fitness = ngram_score('english_quadgrams.txt')
...
def decrypt_two_square(ciphertext, key1, key2): # not working
    return decrypt_playfair(key1, decrypt_playfair(key2, ciphertext))

def bruteforce_two_square(ciphertext): # not working
    fitness = ngram_score('english_quadgrams.txt')
    m = list(alphabet2)
    mScore = -99e9
    p1,p2 = mScore, m[:]
