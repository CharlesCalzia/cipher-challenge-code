...
def decrypt_transposition(key, ciphertext, cipher2=False):
    if not cipher2:
        ...
        return cipher2(ColTrans(key).decipher(ciphertext))


def bruteforce_trans(ciphertext, maxLen=10):
    possible_keys = []
...
def bruteforce_transposition(ciphertext, maxLen=10, cipher2=False):
    for keyLen in range(2, maxLen + 1):
        ...
        for i in range(10):
            ...
            count = 0
            while count < 1000:
                a, b = random.randint(0, keyLen - 1), random.randint(
                    0, keyLen - 1
            ...
            if pScore > mScore:
                ...
                m = p2[:]
                print(
                    f'Best key: {"".join(m)} - {decrypt_transposition("".join(m), ciphertext, cipher2)[:20]}'
                )
