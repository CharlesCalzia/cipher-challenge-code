...
def check_english(plaintext):  # Checks if the text makes sense in english
    try:
        crib = "the"
        ...
        if (
            ...
            and "t" in plaintext
            and "e" in plaintext
            and crib.lower() in plaintext[:60]
        ):
            return True
        else:
            return False
...
def frequency(ciphertext):
    ...
    letter_frequencies = {  # Standard frequency of each letter of the alphabet
        "e": 0.1202,
        "t": 0.0910,
        "a": 0.0812,
        "o": 0.0768,
    ...
    plt.subplot(4, 1, 1)
    plt.bar(
        sorted_freq.keys(), sorted_freq.values(), width=0.3, color="g", align="center"
    )
    plt.bar(
        letter_frequencies.keys(),
        letter_frequencies.values(),
...
def blocks(ciphertext, n=2):  # Split ciphertext into blocks
    blocks = [ciphertext[i : i + n] for i in range(0, len(ciphertext), n)]
    return blocks


def columns(ciphertext, n=2):  # Split ciphertext into columns
